...
(function(ImagePicker, $, undefined) {
	...
	$('select[name=parent_id]').live('change', function() {
		...
		var href_val = SITE_URL + 'streams_core/public_ajax/field/imagepicker/viewpicker/' + folder_id + '/' + (options.showSizeSlider ? '1' : '0') + '/' + (options.showAlignButtons ? '1' : '0') + '/' + (options.fileType ? options.fileType : 'i');
		$('#files_right_pane').load(href_val + ' #files-wrapper', function() {
			$(this).children().fadeIn('slow');
