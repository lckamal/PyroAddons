jQuery(function($){
	...
	$search_results.on('click', 'a', function(e){
		...
		pyro.files.folder_contents(id, type);
	...
	/***************************************************************************
	 * Select file for imagepicker                                             *
	 ***************************************************************************/
	$files_center = $('.folders-center');

	$files_center.on('dblclick', '.file', function(e){
		// store element so it can be accessed the same as if it was right clicked
		pyro.files.$last_r_click = $(e.target).closest('li');
		$('.context-menu-source [data-menu="select_file"]').trigger('click');
	});
	...
	pyro.files.folder_contents = function(folder_id) {
		...
		var type = $('section.item').attr('data-type');

		post_data = { parent : folder_id, type : type };
		$.post(SITE_URL + 'admin/imagepicker/folder_contents', post_data, function(data){
			var results = $.parseJSON(data);
	...
	pyro.files.select_file = function() {
		var timer, location,
			// file or folder?
			type = pyro.files.$last_r_click.hasClass('file') ? 'file' : 'folder',
			// figure out the ID from the last clicked item
			...
			var $imagepicker = parent.ImagePicker;
			if($item.id){
				$imagepicker.close($item);
			}
			else{
				alert('no file selected to insert');
			}
