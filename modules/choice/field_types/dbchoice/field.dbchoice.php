 ...
 */
class Field_dbchoice
{	
	public $field_type_name 		= 'DB Choice';
	public $field_type_slug			= 'dbchoice';
	...
	{
		$choices = $this->_get_db_choices($params['form_slug'], $params['custom']['choice_type'], $field->is_required);
	...
	public function field_assignment_destruct($entry, $field, $stream)
	{
		if(isset($entry->field_slug))
		{
			class_exists('Choice_m') OR ci()->load->model('choice/choice_m');
			ci()->choice_m->delete_by('field_slug', $entry->field_slug);
		}
	}
	...
	{
		$choices = $this->_get_db_choices($data['field_slug'], $data['choice_type'], $data['is_required']);
	...
	{
		return '<p>Choice options are managed from database. <a href="'.site_url('choice').'" target="_blank">Click here</a></p>';
		return array(
	...
	/**
	 * Returns the options on key => value array
	 *
	 * @access	public
	 ...
	 * @return	array
	 */
	public function _get_db_choices($field_slug = null, $type = 'dropdown', $is_required = 'no')
	{
		$choices = array();
		if($field_slug == null)
		{
